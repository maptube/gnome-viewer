...
namespace assets.Scripts
{
    ...
    public class LidarLASReader : MonoBehaviour
    {
        ...
        private float cellsize; //to go with the CoverageData, what is the spacing on the XY base grid - needed, otherwise the height in the value won't match the XY distance (i.e. vertical stretch or shrink)
        ...
        protected void ReadLASPoints(LASPublicHeaderBlock header, ref byte[] data)
        {
            ...
            for (int y = 0; y < isize; y++) for (int x = 0; x < isize; x++) CoverageData[x, y] = 0; // -float.MaxValue;
            ...
            for (ulong i = 0; i < NumPoints; i++)
            {
                ...
                if (val > CoverageData[gx, gy]) CoverageData[gx, gy] = val; //max filter point cloud data
